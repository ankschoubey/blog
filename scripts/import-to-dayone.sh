...
markdown_content=$(sed '1,/^---$/d' "$article_file" | sed '1,/^---$/d')
...
body_images=($(echo "$markdown_content" | grep -o '!\[.*\](/images/.*)' | sed 's/!\[.*\](\(.*\))/public\1/g'))
...
for img_path in "${image_paths[@]}"; do
    ...
    if [ -f "$absolute_path" ]; then
        photo_assets_string+="--attachments $absolute_path "
    else
...
markdown_body=$(awk 'BEGIN{p=0} /^---$/{p++; next} p>=2' "$article_file" | sed '/^import.*;$/d')
