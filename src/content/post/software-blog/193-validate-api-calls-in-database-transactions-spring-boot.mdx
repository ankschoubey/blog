...
We should not make API calls within Database Transactions for increase performance and security. This post is going to talk about:
...
This post is a describes a test first approach to finding the problem. The next post will talk about fixed it. i.e by the end of this post, you'd have a repeatable automated test that can detect this case. In the next blogpost, I will talk about how to separate out API calls from database transaction and different patterns that I have come accross.

Remember: the syntax will change but you can use the same principles in database of your choice.

## Why API calls should not be made within Database Transaction?

The main reason is the database conenciton remains open for a long time. This can cause the following problems:

**Pool Exhausted**

- Database and ORMs want to work efficiently and have a limited connection pool size.
- When a part of code wants to make a DB call, it will acquire a connection and release when it's done using it.
...
- Whenever we open a transaction we open a context in the database and in some cases we lock on particular database rows.
- When this happens, other database transactions won't have access to that row. This can cause timeouts or in worst case a deadlock.

Both of these are security concerns as well because they can be exploite the availability of the service.
...
It is possible to validate that API calls are not within Database transaction right from our Unit Test and Integration Tests with simple test contract without modification to any of our current tests.

In both cases the idea is the following:

1. intercept when a API call is made
2. check in that moment if call is made within transaction
3. if yes, throw an exception

Note while below example is in java, you can replicate in any language.
...
The following is the hibernate code which returns will tell us if call is made with transactional.
...
This was the most exciting part of solving this problem. We want to ensure a [good test pyramid](/software-blog/understanding-testing-pyramid) and have more number of unit test that integration tests to ensure faster tests. But if we aren't using an actual database then how can we tell if call is made within transaction?
...
If the caller of the transaction is in a code construct related to transaction, we throw an exception.

In case of Spring Boot we want to ensure,
...
It's important that API calls are executed outside Database Transactions to ensure that Database isn't locked for a long time. If the database stays locked for a long time, the number of connects end and it will lead to an error.

We can easily validate is API calls are not made within database transactions in both `Integration` test and `Unit` tests.

In the next blogpost is about how to not to make API calls within transaction. You can also checkout my [TDD](/tdd) series for more such validations.
