...
The ViewModel is the single place where DTOs become UI-friendly data. `displayRating` and `isPopular` are computed once, in one file, not scattered across components with ad-hoc format logic.
...
export const useMovie = (id?: string, options?: UseMovieOptions) => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (data: MovieDTO) => options?.onSaveSuccess?.(data.id),
  })
  const updateMutation = useMutation({
...
The container wires the hook and side effects. After saving a new movie, navigate to its detail page. After deleting, go back to the list:
...
function MovieContainer({ id }: { id: string }) {
  ...
  return <MoviePresenter {...movie} />
}
```

The presenter is a pure function of props. No hooks, no side effects:

```tsx
// presenters/MoviePresenter.tsx
function MoviePresenter(props: UseMovieReturn) {
  const { movie, isLoading, canUpdate, updateMovie, isSaving } = props
  ...
  return (
    <div>
      ...
      <button
        ...
        onClick={() => updateMovie({ title: 'New Title' })}
      >
        Save
      </button>
    </div>
...
This separation makes testing straightforward. The hook's return type is the contract, so you test against the shape directly:
...
No mocking of query keys or router state. No rendering through multiple component layers. You test the hook's return value, which is exactly what your presenter receives. Every test follows the same pattern for every feature because every hook returns the same kind of shape.
