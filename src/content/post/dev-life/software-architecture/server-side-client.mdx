...
toc: false
images: https://docs.google.com/drawings/d/e/2PACX-1vTPGz4ZDnTYTiUgt45IqUddwQRgZmyGPvnmHIlanx_AbUq2fgttahG-KxEeGtIBpopJ3v91tLsJacDF/pub?w=1755&h=880
---
...
For example, consider a sports website where a score of a live game is displayed. If thousands of people are on the website, you don't want to read the data from the database each time, because database is designed well for long term storage. You need a data storage that can quickly return value.
...
Caches are in RAM so they are fast. At the same time caches aren't persistent meaning, if a cache server fails, the cache will be lost. Therefore, a lost of caches provide frequent flushing data to disk facility to be fault tolerant.
...
For example, suppose you have 3 instances of a application each reading data from the database.

Whenever they read the database, they cache the value because we expect the value will be read again.
...
If you want to update the data, what should you update first, database or the cache?

This leads us to two stratergies for updating data in cache.

- Write Through
- Write Back

Write through is when you write to the cache first and then write in the database. This is good when you have a real time application.

Write back cache is when you write to the database first then write the updated value in cache. This is simpler as most DBs are heavily constraint meaning if the DB update fails, you don't have to revert the cache.
...
This is one of the ways to implement a write back cache in spring boot.

Spring boot me we have listeners for whenever db chance happens. When that happens we can write to cache.

1. Create a Entity Event Lister which listens to database changes
2. Each time something is save, update the database.
    - You may want to skip update depending on what you use the cache for. If you are using cache for storing frequently used items, you may decide to skip updating cache for less frequent one.
...
Spring Boot also has the following annotation to help with caching
...
Most caches are large key value pairs.

For instance, Redis (likly the most popular distributed cache) only has one giant key value pair.
...
Caches usually have parameters associated with it for eviction. This is because caches are stored in memory (RAM) and these is limited RAM storage.

#### TTL: Time To Live

If a TTL is defined, it means cache this value for this much time. After which remove the cache.
...
![[Mermaid Diagram Aug 04.png]]

### Real Time Applications (Server Side; Distributed)

If there are multiple users using a system in real time, the session would be stored in cache.

For example, assume an online game session like scribble.io. Instead of writing to the database which would be slow, game play data would remain in RAM till the duration of the game.
